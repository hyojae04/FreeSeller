...
- Settings UI에 쿠팡 출고지/반품지/고시정보 입력 필드 추가
- 네이버 카테고리별 고시정보/속성 mapping 추가
- SSG/Lotte/Kakao 공식 partner spec 확보 후 실제 endpoint와 payload 구현
- 연결 테스트는 계속 token-only/read-only API만 사용

검증:

- simulation mode는 기존처럼 mock success
- live mode는 필수 payload 누락 시 등록 API 호출 전 실패
- 연결 테스트는 상품 등록 없이 인증 성공/실패만 판별
